...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, ratings, utils, download, categories, icons
...
def draw_ratings(layout, context):
    ...
    # row = layout.row()
    # op = row.operator("object.blenderkit2_rating_upload", text="Send rating", icon='URL')
    # return op
...
def draw_upload_common(layout, props, asset_type, context):
    op = layout.operator("wm.url_open", text="Read upload instructions",
                         icon='QUESTION')
    if asset_type == 'MODEL':
        op.url = paths.BLENDERKIT2_MODEL_UPLOAD_INSTRUCTIONS_URL
    if asset_type == 'MATERIAL':
        op.url = paths.BLENDERKIT2_MATERIAL_UPLOAD_INSTRUCTIONS_URL
    if asset_type == 'BRUSH':
        op.url = paths.BLENDERKIT2_BRUSH_UPLOAD_INSTRUCTIONS_URL
    ...
    if props.asset_base_id == '':
        ...
        op = layout.operator("object.blenderkit2_upload", text=optext, icon='EXPORT')
        op.asset_type = asset_type

    if props.asset_base_id != '':
        op = layout.operator("object.blenderkit2_upload", text='Reupload asset', icon='EXPORT')
        op.asset_type = asset_type
        op.reupload = True

        op = layout.operator("object.blenderkit2_upload", text='Upload as new asset', icon='EXPORT')
        op.asset_type = asset_type
        op.reupload = False
...
def poll_local_panels():
    user_preferences = bpy.context.preferences.addons['blenderkit2'].preferences
...
def draw_panel_model_upload(self, context):
    ...
    if bpy.context.scene.render.engine in ('CYCLES', 'BLENDER_EEVEE'):
        col.operator("object.blenderkit2_generate_thumbnail", text='Generate thumbnail', icon='IMAGE')
...
def draw_panel_scene_upload(self, context):
    ...
    #     icon = 'ERROR'
    #     layout.operator("object.blenderkit2_auto_tags", text='Auto fill tags', icon=icon)
    # else:
    #     layout.operator("object.blenderkit2_auto_tags", text='Auto fill tags')
    ...
    #     col.operator("object.blenderkit2_generate_thumbnail", text='Generate thumbnail', icon='IMAGE_COL')
...
def draw_assetbar_show_hide(layout, props):
    ...
    op = layout.operator('view3d.blenderkit2_asset_bar', text='', icon=icon)
...
def draw_panel_model_search(self, context):
    ...
    if props.report == 'You need Full plan to get this item.':
        layout.operator("wm.url_open", text="Get Full plan", icon='URL').url = paths.BLENDERKIT2_PLANS
...
class VIEW3D_PT_blenderkit2_model_properties(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_model_properties"
    ...
    def draw(self, context):
        ...
        if o.get('asset_data') is None:
            label_multiline(layout, text='To upload this asset to BlenderKit2, go to the Find and Upload Assets panel.')
            layout.prop(o, 'name')
        ...
        if o.get('asset_data') is not None:
            ...
            if o.instance_type == 'COLLECTION' and o.instance_collection is not None:
                layout.operator('object.blenderkit2_bring_to_scene', text='Bring to scene')

            draw_panel_model_rating(self, context)

            # if 'rig' in ad['tags']:
            #     # layout.label(text = 'can make proxy')
            #     layout.operator('object.blenderkit2_make_proxy', text = 'Make Armature proxy')
        # fast upload, blocked by now
        # else:
        #     op = layout.operator("object.blenderkit2_upload", text='Store as private', icon='EXPORT')
        #     op.asset_type = 'MODEL'
        #     op.fast = True
        # fun override project, not finished
        # layout.operator('object.blenderkit2_color_corrector')


def draw_login_progress(layout):
    layout.label(text='Login through browser')
    layout.label(text='in progress.')
    layout.operator("wm.blenderkit2_login_cancel", text="Cancel", icon='CANCEL')


class VIEW3D_PT_blenderkit2_profile(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_profile"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "BlenderKit2 Profile"
    ...
    def draw(self, context):
        ...
        if user_preferences.api_key != '':
                ...
                if me.get('currentPlanName') is not None:
                    ...
                    if pn =='Free':
                        layout.operator("wm.url_open", text="Change plan",
                            icon='URL').url = paths.get_bkit_url() + paths.BLENDERKIT2_PLANS
            ...
            layout.operator("wm.url_open", text="See my uploads",
                            icon='URL').url = paths.get_bkit_url() + paths.BLENDERKIT2_USER_ASSETS


class VIEW3D_PT_blenderkit2_login(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_login"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "BlenderKit2 Login"
...
def draw_panel_material_upload(self, context):
    ...
    if bpy.context.scene.render.engine in ('CYCLES', 'BLENDER_EEVEE'):
        layout.operator("object.blenderkit2_material_thumbnail", text='Render thumbnail with Cycles', icon='EXPORT')
...
def draw_login_buttons(layout):
    ...
    if user_preferences.login_attempt:
        ...
        if user_preferences.api_key == '':
            layout.operator("wm.blenderkit2_login", text="Login",
                            icon='URL').signup = False
            layout.operator("wm.blenderkit2_login", text="Sign up",
                            icon='URL').signup = True

        else:
            layout.operator("wm.blenderkit2_login", text="Login as someone else",
                            icon='URL').signup = False
            layout.operator("wm.blenderkit2_logout", text="Logout",
                            icon='URL')


class VIEW3D_PT_blenderkit2_advanced_model_search(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_advanced_model_search"
    # bl_parent_id = "VIEW3D_PT_blenderkit2_unified"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
...
class VIEW3D_PT_blenderkit2_unified(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_unified"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    ...
    def draw(self, context):
        ...
        if len(user_preferences.api_key) < 20 and user_preferences.asset_counter > 20:
            if user_preferences.enable_oauth:
                ...
                op = layout.operator("wm.url_open", text="Get your API Key",
                                     icon='QUESTION')
                op.url = paths.BLENDERKIT2_SIGNUP_URL
        ...
        if ui_props.down_up == 'SEARCH':
            ...
            op = layout.operator('view3d.blenderkit2_asset_bar', text=text, icon='EXPORT')
            op.keep_running = False
            op.do_search = False
            ...
            if e not in ('CYCLES', 'BLENDER_EEVEE'):
                rtext = 'Only Cycles and EEVEE render engines are currently supported. ' \
                        'Please use Cycles for all assets you upload to BlenderKit2.'
            ...
            if ui_props.asset_type == 'MATERIAL':
                if bpy.context.view_layer.objects.active is not None and \
                        ...
                        bpy.context.active_object.active_material.blenderkit2.asset_base_id != '':
                    layout.label(text=bpy.context.active_object.active_material.blenderkit2.name + ' :')
                    # noinspection PyCallByClass
...
class OBJECT_MT_blenderkit2_asset_menu(bpy.types.Menu):
    bl_label = "Asset options:"
    bl_idname = "OBJECT_MT_blenderkit2_asset_menu"

    def draw(self, context):
        layout = self.layout
        ui_props = context.scene.blenderkit2UI
        ...
        if wm.get('bkit authors') is not None:
            ...
            if a is not None:
                ...
                op = layout.operator('view3d.blenderkit2_search', text="Show Assets By Author")
                op.keywords = ''
                op.author_id = author_id

        op = layout.operator('view3d.blenderkit2_search', text='Search Similar')
        ...
        if asset_data.get('canDownload') != 0:
            if bpy.context.view_layer.objects.active is not None and ui_props.asset_type == 'MODEL':
                ...
                op = layout.operator('scene.blenderkit2_download', text='Replace Active Models')
        ...
        if profile is not None:
            ...
            if utils.profile_is_validator():
                ...
                if asset_data['verificationStatus'] != 'uploaded':
                    op = layout.operator('object.blenderkit2_change_status', text='set Uploaded')
                    op.asset_id = asset_data['id']
                    op.state = 'uploaded'
                if asset_data['verificationStatus'] != 'validated':
                    op = layout.operator('object.blenderkit2_change_status', text='Validate')
                    op.asset_id = asset_data['id']
                    op.state = 'validated'
                if asset_data['verificationStatus'] != 'on_hold':
                    op = layout.operator('object.blenderkit2_change_status', text='Put on Hold')
                    op.asset_id = asset_data['id']
                    op.state = 'on_hold'
                if asset_data['verificationStatus'] != 'rejected':
                    op = layout.operator('object.blenderkit2_change_status', text='Reject')
                    op.asset_id = asset_data['id']
                    op.state = 'rejected'
            ...
            if author_id == str(profile['user']['id']):
                ...
                op = row.operator('object.blenderkit2_change_status', text='Delete')
                op.asset_id = asset_data['id']
                op.state = 'deleted'
            # else:
...
class SetCategoryOperator(bpy.types.Operator):
    """Visit subcategory"""
    bl_idname = "view3d.blenderkit2_set_category"
    bl_label = "BlenderKit2 Set Active Category"
...
class UrlPopupDialog(bpy.types.Operator):
    ...
    bl_idname = "wm.blenderkit2_url_dialog"
    bl_label = "BlenderKit2 message:"
...
def draw_panel_categories(self, context):
    ...
    if wm.get('active_category') is not None:
        ...
        if len(acat) > 1:
            ...
            op = col.operator('view3d.blenderkit2_set_category', text='...', icon='FILE_PARENT')
            op.asset_type = ui_props.asset_type
            op.category = ''
    ...
    #     op = col.operator('view3d.blenderkit2_asset_bar', text='freebies')
    #     op.free_only = True
    ...
    for c in cats['children']:
        if c['assetCount'] > 0:
            ...
            op = row.operator('view3d.blenderkit2_asset_bar', text=ctext)
            op.do_search = True
            ...
            if len(c['children']) > 0 and c['assetCount'] > 15:
                # row = row.split()
                op = row.operator('view3d.blenderkit2_set_category', text='>>')
                op.asset_type = ui_props.asset_type
                op.category = c['slug']
                ...
                #         op = row.operator('view3d.blenderkit2_search', text=ctext)
                #         op.category = c1['slug']


class VIEW3D_PT_blenderkit2_downloads(Panel):
    bl_category = "BlenderKit2"
    bl_idname = "VIEW3D_PT_blenderkit2_downloads"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = "Downloads"
    ...
    def draw(self, context):
        ...
        for threaddata in download.download_threads:
            ...
            row.operator('scene.blenderkit2_download_kill', text='', icon='CANCEL')
...
def header_search_draw(self, context):
    ...
    preferences = bpy.context.preferences.addons['blenderkit2'].preferences
    if preferences.search_in_header:
        ...
        if ui_props.asset_type == 'MODEL':
            props = s.blenderkit2_models
        if ui_props.asset_type == 'MATERIAL':
            props = s.blenderkit2_mat
        if ui_props.asset_type == 'BRUSH':
            props = s.blenderkit2_brush
...
classess = (
    SetCategoryOperator,
    VIEW3D_PT_blenderkit2_profile,
    VIEW3D_PT_blenderkit2_login,
    VIEW3D_PT_blenderkit2_unified,
    # VIEW3D_PT_blenderkit2_advanced_model_search,
    VIEW3D_PT_blenderkit2_model_properties,
    VIEW3D_PT_blenderkit2_downloads,
    OBJECT_MT_blenderkit2_asset_menu,
    UrlPopupDialog
)
