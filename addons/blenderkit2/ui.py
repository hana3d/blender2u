...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, ratings, utils, search, upload, ui_bgl, download, bg_blender, colors, tasks_queue
...
def draw_ratings_bgl():
    # return;
    ui = bpy.context.scene.blenderkit2UI
...
def draw_callback_2d(self, context):
    ...
    if go and a == a1 and w == w1:
        ...
        props = context.scene.blenderkit2UI
...
def draw_callback_3d(self, context):
    ''' Draw snapped bbox while dragging and in the future other blenderkit2 related stuff. '''
    if not utils.guard_from_crash():
        return;

    ui = context.scene.blenderkit2UI
...
def mouse_raycast(context, mx, my):
    ...
    if has_hit:
        ...
        up = Vector((0, 0, 1))
        props = bpy.context.scene.blenderkit2_models
...
class AssetBarOperator(bpy.types.Operator):
    ...
    bl_idname = "view3d.blenderkit2_asset_bar"
    bl_label = "BlenderKit2 Asset Bar UI"
    ...
    def modal(self, context, event):
        ...
        if event.type == 'WHEELUPMOUSE' or event.type == 'WHEELDOWNMOUSE' or event.type == 'TRACKPADPAN':
            ...
            if ui_props.dragging and not mouse_in_asset_bar(mx, my):  # and my < r.height - ui_props.bar_height \
                ...
                sprops = bpy.context.scene.blenderkit2_models
        ...
        if event.type == 'MOUSEMOVE':  # Apply
            ...
            if not ui_props.dragging:
                ...
                if asset_search_index > -1:
                    ...
                    # bpy.ops.wm.call_menu(name='OBJECT_MT_blenderkit2_asset_menu')
            ...
            if event.value == 'PRESS' and ui_props.active_index > -1:
                if ui_props.asset_type == 'MODEL' or ui_props.asset_type == 'MATERIAL':
                    ...
                    if not asset_data['can_download']:
                        ...
                        url = paths.get_bkit_url() + '/get-blenderkit2/' + asset_data['id'] + '/?from_addon'
                        bpy.ops.wm.blenderkit2_url_dialog('INVOKE_REGION_WIN', url=url, message=message,
                                                         link_text=link_text)
                        return {'RUNNING_MODAL'}
                ...
                if asset_search_index > -3:
                    if ui_props.asset_type == 'MATERIAL':
                        if target_object != '':
                            ...
                            bpy.ops.scene.blenderkit2_download(True,
                        ...
                        bpy.ops.scene.blenderkit2_download(asset_type=ui_props.asset_type,
                                                          asset_index=asset_search_index)
...
class TransferBlenderkit2Data(bpy.types.Operator):
    """Regenerate cobweb"""
    bl_idname = "object.blenderkit2_data_trasnfer"
    bl_label = "Transfer BlenderKit2 data"
    bl_description = "Transfer blenderKit2 metadata from one object to another when fixing uploads with wrong parenting."
    ...
    def execute(self, context):
        ...
        for target_ob in bpy.context.selected_objects:
            if target_ob != source_ob:
                target_ob.property_unset('blenderkit2')
                for k in source_ob.keys():
                    target_ob[k] = source_ob[k]
        source_ob.property_unset('blenderkit2')
        return {'FINISHED'}
...
class UndoWithContext(bpy.types.Operator):
    ...
    bl_description = "BlenderKit2 undo push with fixed context"
    bl_options = {'REGISTER', 'UNDO', 'INTERNAL'}

    # def modal(self, context, event):
    #     return {'RUNNING_MODAL'}

    message: StringProperty('Undo Message', default='BlenderKit2 operation')
...
class RunAssetBarWithContext(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.area is None or context.area.type != 'VIEW_3D':
            ...
            override = {'window': w, 'screen': w.screen, 'area': a, 'region': r}
            C_dict.update(override)
        bpy.ops.view3d.blenderkit2_asset_bar(C_dict, 'INVOKE_REGION_WIN', keep_running=True, do_search=False)
        return {'FINISHED'}
