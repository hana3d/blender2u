...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, append_link, utils, ui, colors, tasks_queue, rerequests
...
def scene_save(context):
    ''' does cleanup of blenderkit2 props and sends a message to the server about assets used.'''
...
def report_usages():
    ...
    url = paths.get_api_url() + paths.BLENDERKIT2_REPORT_URL
...
def append_asset(asset_data, **kwargs):  # downloaders=[], location=None,
    ...
    if asset_data['asset_type'] == 'model':
        ...
        for b in bpy.data.brushes:
            ...
            if b.blenderkit2.id == asset_data['id']:
                inscene = True
                brush = b
                break;
        if not inscene:
        ...
        for m in bpy.data.materials:
            if m.blenderkit2.id == asset_data['id']:
                inscene = True
                material = m
                break;
        if not inscene:
    ...
    parent['asset_data'] = asset_data  # TODO remove this??? should write to blenderkit2 Props?
...
def timer_update():  # TODO might get moved to handle all blenderkit2 stuff, not to slow down.
...
class Downloader(threading.Thread):
    ...
    def run(self):
        '''try to download file from blenderkit2'''
...
class Blenderkit2KillDownloadOperator(bpy.types.Operator):
    """Kill a download"""
    bl_idname = "scene.blenderkit2_download_kill"
    bl_label = "BlenderKit2 Kill Asset Download"
...
class Blenderkit2DownloadOperator(bpy.types.Operator):
    """Download and link asset to scene. Only link if asset already available locally."""
    bl_idname = "scene.blenderkit2_download"
    bl_label = "BlenderKit2 Asset Download"
    ...
    #     return bpy.context.window_manager.BlenderKit2ModelThumbnails is not ''
...
def register_download():
    bpy.utils.register_class(Blenderkit2DownloadOperator)
    bpy.utils.register_class(Blenderkit2KillDownloadOperator)
...
def unregister_download():
    bpy.utils.unregister_class(Blenderkit2DownloadOperator)
    bpy.utils.unregister_class(Blenderkit2KillDownloadOperator)
