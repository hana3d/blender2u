...
if "bpy" in locals():
    ...
    from blenderkit2 import tasks_queue, utils, paths, search, categories, oauth, ui
...
def write_tokens(auth_token, refresh_token, oauth_response):
    ...
    if props is not None:
        props.report = ''
    ui.add_report('BlenderKit2 Re-Login success')
    search.get_profile()
    categories.fetch_categories_thread(auth_token)


class RegisterLoginOnline(bpy.types.Operator):
    """Login online on BlenderKit2 webpage"""

    bl_idname = "wm.blenderkit2_login"
    bl_label = "BlenderKit2 login or signup"
...
class Logout(bpy.types.Operator):
    """Logout from BlenderKit2 immediately"""

    bl_idname = "wm.blenderkit2_logout"
    bl_label = "BlenderKit2 logout"
    bl_options = {'REGISTER', 'UNDO'}
...
class CancelLoginOnline(bpy.types.Operator):
    ...
    bl_idname = "wm.blenderkit2_login_cancel"
    bl_label = "BlenderKit2 login cancel"
    bl_options = {'REGISTER', 'UNDO'}
