...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, utils, categories, ui, colors, bkit_oauth, version_checker, tasks_queue, rerequests
...
def timer_update():
    ...
    if first_time:  # first time
        ...
        if preferences.tips_on_start:
            ...
            ui.add_report(text='BlenderKit2 Tip: ' + random.choice(rtips), timeout=12, color=colors.GREEN)
        return 3.0
    ...
    if bpy.context.scene.blenderkit2UI.dragging:
        return 0.5
...
class Searcher(threading.Thread):
    ...
    def query_to_url(self):
        ...
        # result ordering: _score - relevance, score - BlenderKit2 score
...
def build_query_scene():
    ...
    props = bpy.context.scene.blenderkit2_scene
    query = {
        "asset_type": 'scene',
        # "engine": props.search_engine,
        # "adult": props.search_adult,
...
def build_query_material():
    props = bpy.context.scene.blenderkit2_mat
    query = {
        "asset_type": 'material',
...
def build_query_texture():
    props = bpy.context.scene.blenderkit2_tex
    query = {
        "asset_type": 'texture',
...
def build_query_brush():
    props = bpy.context.scene.blenderkit2_brush
...
def search(category='', get_next=False, author_id=''):
    ...
    # mt('start')
    scene = bpy.context.scene
    uiprops = scene.blenderkit2UI

    if uiprops.asset_type == 'MODEL':
        if not hasattr(scene, 'blenderkit2'):
            return;
        props = scene.blenderkit2_models
        query = build_query_model()

    if uiprops.asset_type == 'SCENE':
        if not hasattr(scene, 'blenderkit2_scene'):
            return;
        props = scene.blenderkit2_scene
        query = build_query_scene()

    if uiprops.asset_type == 'MATERIAL':
        if not hasattr(scene, 'blenderkit2_mat'):
            return;
        props = scene.blenderkit2_mat
        query = build_query_material()

    if uiprops.asset_type == 'TEXTURE':
        if not hasattr(scene, 'blenderkit2_tex'):
            return;
        # props = scene.blenderkit2_tex
        # query = build_query_texture()

    if uiprops.asset_type == 'BRUSH':
        if not hasattr(scene, 'blenderkit2_brush'):
            return;
        props = scene.blenderkit2_brush
    ...
    tasks_queue.add_task((ui.add_report, ('BlenderKit2 searching....', 2)))

    props.report = 'BlenderKit2 searching....'
...
class SearchOperator(Operator):
    """Tooltip"""
    bl_idname = "view3d.blenderkit2_search"
    bl_label = "BlenderKit2 asset search"
    ...
    def execute(self, context):
        ...
        # bpy.ops.view3d.blenderkit2_asset_bar()
