...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, utils, bg_blender

import tempfile, os, subprocess, json, sys

import bpy

BLENDERKIT2_EXPORT_DATA_FILE = "data.json"
...
def start_thumbnailer(self, context):
    ...
    mainmodel.blenderkit2.is_generating_thumbnail = True
    mainmodel.blenderkit2.thumbnail_generating_state = 'starting blender instance'
    ...
    filepath = os.path.join(tempdir, "thumbnailer_blenderkit2" + ext)
    tfpath = paths.get_thumbnailer_filepath()
    datafile = os.path.join(tempdir, BLENDERKIT2_EXPORT_DATA_FILE)
    try:
        ...
        eval_path_computing = "bpy.data.objects['%s'].blenderkit2.is_generating_thumbnail" % mainmodel.name
        eval_path_state = "bpy.data.objects['%s'].blenderkit2.thumbnail_generating_state" % mainmodel.name
        ...
        mainmodel.blenderkit2.thumbnail = rel_thumb_path + '.jpg'
        mainmodel.blenderkit2.thumbnail_generating_state = 'Saving .blend file'
...
def start_material_thumbnailer(self, context):
    # Prepare to save the file
    mat = bpy.context.active_object.active_material
    mat.blenderkit2.is_generating_thumbnail = True
    mat.blenderkit2.thumbnail_generating_state = 'starting blender instance'
    ...
    try:
        ...
        eval_path_computing = "bpy.data.materials['%s'].blenderkit2.is_generating_thumbnail" % mat.name
        eval_path_state = "bpy.data.materials['%s'].blenderkit2.thumbnail_generating_state" % mat.name
        eval_path = "bpy.data.materials['%s']" % mat.name

        bg_blender.add_bg_process(eval_path_computing=eval_path_computing, eval_path_state=eval_path_state,
                                  eval_path=eval_path, process_type='THUMBNAILER', process=proc)

        mat.blenderkit2.thumbnail = rel_thumb_path + '.png'
        mat.blenderkit2.thumbnail_generating_state = 'Saving .blend file'
    except Exception as e:
...
class GenerateThumbnailOperator(bpy.types.Operator):
    ...
    bl_idname = "object.blenderkit2_generate_thumbnail"
    bl_label = "BlenderKit2 Thumbnail Generator"
...
class GenerateMaterialThumbnailOperator(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "object.blenderkit2_material_thumbnail"
    bl_label = "BlenderKit2 Material Thumbnail Generator"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def draw(self, context):
        layout = self.layout
        props = bpy.context.active_object.active_material.blenderkit2
