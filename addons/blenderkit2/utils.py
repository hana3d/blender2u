...
def get_selected_models():
    ...
    for ob in obs:
        if ob not in done:
            while ob.parent is not None and ob not in done and ob.blenderkit2.asset_base_id != '' and ob.instance_collection is not None:
                done[ob] = True
                ob = ob.parent

            if ob not in parents and ob not in done:
                if ob.blenderkit2.name != '' or ob.instance_collection is not None:
                    parents.append(ob)
            done[ob] = True

    #if no blenderkit2 - like objects were found, use the original selection.
    if len(parents) == 0:
        parents = obs
    return parents
...
def get_search_props():
    ...
    if uiprops.asset_type == 'MODEL':
        if not hasattr(scene, 'blenderkit2_models'):
            return;
...
def get_upload_props():
    ...
    if ui_props.asset_type == 'MODEL':
        if bpy.context.view_layer.objects.active is not None:
            ob = get_active_model()
            return ob.blenderkit2
    if ui_props.asset_type == 'SCENE':
        s = bpy.context.scene
        return s.blenderkit2
    elif ui_props.asset_type == 'MATERIAL':
        if bpy.context.view_layer.objects.active is not None and bpy.context.active_object.active_material is not None:
            return bpy.context.active_object.active_material.blenderkit2
    elif ui_props.asset_type == 'TEXTURE':
        return None
    elif ui_props.asset_type == 'BRUSH':
        b = get_active_brush()
        if b is not None:
            return b.blenderkit2
    return None
...
def load_prefs():
    ...
    fpath = paths.BLENDERKIT2_SETTINGS_FILENAME
...
def automap(target_object=None, target_slot=None, tex_size=1, bg_exception=False, just_scale = False):
    from blenderkit2 import bg_blender as bg
    s = bpy.context.scene
    mat_props = s.blenderkit2_mat
...
def guard_from_crash():
    ...
    if bpy.context.preferences.addons.get('blenderkit2') is None:
        return False;
    if bpy.context.preferences.addons['blenderkit2'].preferences is None:
        return False;
    return True
