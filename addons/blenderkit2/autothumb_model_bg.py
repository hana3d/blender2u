...
if "bpy" in locals():
    ...
    from blenderkit2 import utils, append_link, bg_blender
...
BLENDERKIT2_EXPORT_TEMP_DIR = sys.argv[-1]
BLENDERKIT2_THUMBNAIL_PATH = sys.argv[-2]
BLENDERKIT2_EXPORT_FILE_INPUT = sys.argv[-3]
BLENDERKIT2_EXPORT_DATA = sys.argv[-4]


def get_obnames():
    with open(BLENDERKIT2_EXPORT_DATA, 'r') as s:
        data = json.load(s)
...
if __name__ == "__main__":
    try:
        ...
        obnames = get_obnames()
        main_object, allobs = append_link.append_objects(file_name=BLENDERKIT2_EXPORT_FILE_INPUT,
                                                         obnames=obnames,
                                                         link=True)
        bpy.context.view_layer.update()
        ...
        bpy.context.scene.render.filepath = BLENDERKIT2_THUMBNAIL_PATH
        ...
        render_thumbnails()
        fpath = BLENDERKIT2_THUMBNAIL_PATH + '0001.jpg'
