...
if "bpy" in locals():
    ...
    from blenderkit2 import paths, utils, rerequests, tasks_queue
...
class StarRatingOperator(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "object.blenderkit2_rating"
...
class UploadRatingOperator(bpy.types.Operator):
    ...
    bl_idname = "object.blenderkit2_rating_upload"
...
def draw_rating(layout, props, prop_name, name):
    ...
    for a in range(0, 10):
        ...
        op = row.operator('object.blenderkit2_rating', icon=icon, emboss=False, text='')
