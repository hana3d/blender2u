...
if "bpy" in locals():
    ...
    from blenderkit2 import asset_inspector, paths, utils, bg_blender, autothumb, version_checker, search, ui_panels, ui, \
...
def get_upload_data(self, context, asset_type):
    ...
    if asset_type == 'MODEL':
        ...
        eval_path_computing = "bpy.data.objects['%s'].blenderkit2.uploading" % mainmodel.name
        eval_path_state = "bpy.data.objects['%s'].blenderkit2.upload_state" % mainmodel.name
    ...
    if asset_type == 'SCENE':
        ...
        eval_path_computing = "bpy.data.scenes['%s'].blenderkit2.uploading" % s.name
        eval_path_state = "bpy.data.scenes['%s'].blenderkit2.upload_state" % s.name
        ...
        eval_path_computing = "bpy.data.materials['%s'].blenderkit2.uploading" % mat.name
        eval_path_state = "bpy.data.materials['%s'].blenderkit2.upload_state" % mat.name
        ...
        eval_path_computing = "bpy.data.brushes['%s'].blenderkit2.uploading" % brush.name
        eval_path_state = "bpy.data.brushes['%s'].blenderkit2.upload_state" % brush.name
...
def start_upload(self, context, asset_type, reupload, upload_set):
    ...
    source_filepath = os.path.join(tempdir, "export_blenderkit2" + ext)
...
class UploadOperator(Operator):
    """Tooltip"""
    bl_idname = "object.blenderkit2_upload"
    bl_description = "Upload or re-upload asset + thumbnail + metadata"

    bl_label = "BlenderKit2 asset upload"
    ...
    def execute(self, context):
        bpy.ops.object.blenderkit2_auto_tags()
        props = utils.get_upload_props()
...
class AssetVerificationStatusChange(Operator):
    ...
    bl_idname = "object.blenderkit2_change_status"
    ...
    def draw(self, context):
        ...
        layout.label(text='Really delete asset from BlenderKit2 online storage?')
