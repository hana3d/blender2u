...
def get_libraries():
    libs = [Library(matlib_path, f)
            for f in os.listdir(matlib_path) if f[-5::] == "blend"]
    try:
        user_path = bpy.context.preferences.addons[__name__].preferences.matlib_path
        if user_path:
            if os.path.exists(user_path):
                libs.extend([Library(user_path, f)
                             for f in os.listdir(user_path) if f[-5::] == "blend"])
            else:
...
class matlibProperties(PropertyGroup):
    ...
    search: StringProperty(
        name="Search", description="Find By Name", update=update_search_index)
    ...
    link: BoolProperty(
        name="Linked", description="Link the material", default=False)
    force_import: BoolProperty(
        name="Force Import", description="Use Scene Materials by default", default=False)
    filter: BoolProperty(
        name="Filter", description="Filter Categories", default=True, update=update_filter)
    show_prefs: BoolProperty(
        name="show_prefs", description="Preferences", default=False)
    last_selected: StringProperty(name="Last Selected")
    hide_search: BoolProperty(
        name="Hide Search", description="Use Blender Search Only")
...
""" % (mat.name, cat, winpath(self.current_library.path))
    ...
    def apply(self, context, preview=False):
        ...
        # apply
            ...
            objects = [obj for obj in context.selected_objects if hasattr(
                obj.data, "materials")]
...
'''.format(overwrite, thispath, name, libpath)
        ...
        if send_command(cmd):
            ...
            if not overwrite:
                ...
                items = sorted([[item.name, item.category]
                                for item in self.all_materials], key=lambda x: x[0])
...
class MATLIB_MT_LibsMenu(Menu):
    ...
    def draw(self, context):
        ...
        for i, lib in enumerate(libs):
            layout.operator("matlib.operator",
                            text=lib.shortname).cmd = "lib" + str(i)
...
class MATLIB_OT_reload(Operator):
    """Reload library"""
...
class MATLIB_OT_apply(Operator):
    ...
    def poll(cls, context):
        matlib = context.scene.matlib
        index = matlib.mat_index
        length = len(matlib.materials)
        obj = context.active_object
        return length > 0 and index >= 0 and index < length and obj is not None
...
class MATLIB_OT_preview(Operator):
    ...
    def poll(cls, context):
        ...
        length = len(matlib.materials)
        # obj = context.active_object
        return length > 0 and index >= 0 and index < length
...
class MATLIB_PT_vxPanel(Panel):
    ...
    def draw(self, context):
        ...
        split.operator("matlib.operator", icon="ADD",
