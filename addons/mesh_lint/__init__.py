...
try:
    ...
    class MeshLintAnalyzer:
        ...
        CHECKS.append({
            'symbol': 'tris',
            'label': 'Tris',
            'definition': 'A face with 3 edges. Often bad for modeling because it stops edge loops and does not deform well around bent areas. \
        ...
        CHECKS.append({
            ...
            'definition': 'Simply, shapes that won\'t hold water. More precisely, nonmanifold edges are those that do not have exactly 2 \
                           faces attached to them (either more or less). Nonmanifold verts are more complicated -- you can see their definition in \
                           BM_vert_is_manifold() in bmesh_queries.c',
            'default': True
        })
        ...
        CHECKS.append({
            ...
            'definition': 'This confuses people. It is very specific: A face whose edges ALL have >2 faces attached. The simplest way to see this \
                           is to Ctrl+r a Default Cube and hit \'f\'',
            'default': True
        })
        ...
        CHECKS.append({
            ...
            'label': '6+-edge Poles',
            'definition': 'A vertex with 6 or more edges connected to it. Generally this is not something you want, but since some kinds \
                           of extrusions will legitimately cause such a pole (imagine extruding each face of a Cube outward, the inner \
                           corners are rightful 6+-poles). Still, if you don\'t know for sure that you want them, it is good to enable this',
            'default': True
        })
    ...
    class MeshLintContinuousChecker():
        ...
        def check(cls):
            ...
            if None is not previous_topology_counts:
    ...
    class MESH_PT_MeshLintControl(bpy.types.Panel):
        ...
        def is_bad_name(cls, name):
            ...
            pat = r'(%s)\.?\d*$' % '|'.join(default_names)
            return None is not re.match(pat, name)
        ...
        class TestAnalysis(unittest.TestCase):
            ...
            def test_comparison(self):
                ...
                self.assertEqual(
                    ...
                    MeshLintContinuousChecker.diff_analyses(
                        [
                            {'lint': {'label': 'Tris'}, 'verts': [], 'edges': [1, 4], 'faces': [], },
                            {'lint': {'label': 'CheckB'}, 'verts': [], 'edges': [2, 3], 'faces': [], },
                            {'lint': {'label': 'Nonmanifold Elements'}, 'verts': [], 'edges': [], 'faces': [2, 3], },
                        ],
                        [
                            {'lint': {'label': 'Tris'}, 'verts': [], 'edges': [1, 4, 5, 6], 'faces': [], },
                            {'lint': {'label': 'CheckB'}, 'verts': [], 'edges': [2, 3], 'faces': [], },
                            {'lint': {'label': 'Nonmanifold Elements'}, 'verts': [1, 2, 3, 4], 'edges': [], 'faces': [2, 3, 5], },
                        ]),
                ...
                self.assertEqual(
                    ...
                    MeshLintContinuousChecker.diff_analyses(
                        [
                            {'lint': {'label': '6+-edge Poles'}, 'verts': [], 'edges': [2, 3], 'faces': [], },
                            {'lint': {'label': 'Nonmanifold Elements'}, 'verts': [], 'edges': [2, 3], 'faces': [], },
                        ],
                        [
                            {'lint': {'label': 'Tris'}, 'verts': [55], 'edges': [], 'faces': [], },
                            {'lint': {'label': 'Ngons'}, 'verts': [], 'edges': [], 'faces': [5, 6], },
                            {'lint': {'label': 'Nonmanifold Elements'}, 'verts': [], 'edges': [2, 3, 4, 5], 'faces': [], },
                        ]),
